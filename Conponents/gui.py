...
class FiniteAutomatonGUI:
    def __init__(self, root):
        ...
        self.transitions_text = ctk.CTkTextbox(self.frame)
        self.transitions_text.grid(row=4, column=1)

        # Create buttons
        ctk.CTkButton(self.frame, text="Create FA", command=self.create_fa).grid(row=5, column=0)
        ctk.CTkButton(self.frame, text="Check Determinism", command=self.check_determinism).grid(row=6, column=0)
        ctk.CTkButton(self.frame, text="Check String Acceptance", command=self.check_string_acceptance).grid(row=7, column=0)
        ctk.CTkButton(self.frame, text="Convert to DFA", command=self.convert_to_dfa).grid(row=8, column=0)
        ctk.CTkButton(self.frame, text="Minimize DFA", command=self.minimize_dfa).grid(row=9, column=0)

        # Image display for visualization
        self.image_label = ctk.CTkLabel(self.frame)
        self.image_label.grid(row=10, column=0, columnspan=2)
    ...
    def visualize_fa(self):
        try:
            dot = self.fa.to_dot()
            dot.render('finite_automaton', format='png')
            img = Image.open('finite_automaton.png')
